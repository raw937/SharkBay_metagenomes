venn_diagram2 <- function(a, b, name_a = "A", name_b = "B", colors =  c("#B80830","#EACC33"), name_output = "default.pdf", euler = FALSE){
	# Niels Hanson
	...
	temp <- list(
		name_a = c(1:length(a)),
		name_b = c((length(a_only)+1):((length(a_only)-1)+length(b)))
		)
