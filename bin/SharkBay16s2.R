...
data <- read.table("SB16SformatEdit.txt", sep="\t", header=T)
data.m <- melt(data)

#Plot Dot Plot
ggplot(data.m, aes(x=variable, y=phylum, color=variable)) + geom_point(aes(size=log(value))) + theme_bw() + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 1))
